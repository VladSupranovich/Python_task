...
@pytest.fixture
def test_db():
    db = python_task.DataBase(db_host=config.db_host, db_name=config.db_name, db_user=config.db_user, db_pass=config.db_pass, db_port=config.db_port)
    yield db
    db.conn.dispose()
...
class TestConnection:
    ...
    def test_db_connection_unsuccessful(self):
        db = python_task.DataBase(db_host="localhost", db_name="python_task", db_user="postgres", db_pass="1234", db_port="5432")
...
class TestWriteFile:
    def test_write_file_successful(self, test_db):
        ...
        with open('test_file.json', 'w') as test_file:
            test_file.write('[{"name": "John", "age": 30}, {"name": "Jane", "age": 25}]')
...
class TestSelectData:
    def test_select_data_json(self, test_db):
        ...
        format = 'json'
        test_db.select_data(query, format)
        assert os.path.isfile(f'query_result/query.{format}')
        data = pd.read_json(f'query_result/query.{format}')
        assert len(data) == 2
    ...
    def test_select_data_xml(self, test_db):
        ...
        test_db.select_data(query, format)
        assert os.path.isfile(f'query_result/query.{format}')
        data = pd.read_xml(f'query_result/query.{format}')
        assert len(data) == 2
        assert data['name'][0] == 'John'
        assert data['age'][0] == 30
    ...
    def test_select_data_format_error(self, test_db):
        ...
        query = "SELECT * FROM test_table"
        format = 'sql'
        assert test_db.select_data(query, format) == False
